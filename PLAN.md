...
- **Grafana: written** (`grafana/sump-dashboard.json`, generated by
  `tools/make_dashboard.py`, and the "Sump monitor silent" alert in
  `grafana/sump-alerting.yaml`), not yet imported or tested against real data.
- **To do:** hardware bring-up and calibration (pit dimensions, pump on/off
  levels).
